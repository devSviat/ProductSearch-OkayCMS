...
```bash
git clone https://github.com/devSviat/ProductSearch-OkayCMS.git
```
...
```
{OkayCMS_root}/
├── Okay/Modules/Sviat/ProductSearch/     ← модуль
└── tests/Modules/Sviat/ProductSearch/     ← його тести
```
...
```bash
cd {OkayCMS_root}
mkdir -p tests/Modules/Sviat/ProductSearch
cp -r ~/ProductSearch-OkayCMS/tests/. tests/Modules/Sviat/ProductSearch/
```
...
```bash
cd {OkayCMS_root}
vendor/bin/phpunit tests/Modules/Sviat/ProductSearch
```
...
2. Неймспейс — `Modules\Sviat\ProductSearch`.
...
use Okay\Modules\Sviat\ProductSearch\Services\SomeService;
